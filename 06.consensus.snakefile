...
rule all:
  input:
    ...
    [f'{CONSENSUS_PATH}/{sample}/{RUNID}-{sample_and_ref[0]}-{sample_and_ref[1]}-{covlimit}x-consensus.csv' for sample_and_ref, covlimit in product(COV_LIMIT, zip(SAMPLES, REF))],
    [f'{CONSENSUS_PATH}/{sample}/{RUNID}-{sample_and_ref[0]}-{sample_and_ref[1]}-{covlimit}x-alignment-file.csv'for sample_and_ref, covlimit in product(COV_LIMIT, zip(SAMPLES, REF))],
